module github.com/sebastianmontero/hypha-document-cache-gql-go

go 1.15

require (
	github.com/dfuse-io/bstream v0.0.2-0.20210105170217-db7e8fd1e9ed
	github.com/dfuse-io/client-go v0.0.0-20210111154104-a57a0b7a63fc // indirect
	github.com/dfuse-io/dfuse-eosio v0.1.1-docker.0.20210106190033-47b917933e19
	github.com/dfuse-io/pbgo v0.0.6-0.20210108215028-712d6889e94a
	github.com/iancoleman/strcase v0.1.3
	github.com/machinebox/graphql v0.2.2
	github.com/matryer/is v1.4.0 // indirect
	github.com/prometheus/client_golang v1.2.1
	github.com/rs/zerolog v1.20.0
	github.com/sebastianmontero/dfuse-firehose-client v0.0.0-20220521193606-1b5d285035af
	github.com/sebastianmontero/dgraph-go-client v0.0.0-20210213215931-344d1e456654
	github.com/sebastianmontero/slog-go v0.0.0-20210213204103-60eda76e8d74
	github.com/spf13/viper v1.8.0
	github.com/vektah/gqlparser v1.3.1
	gotest.tools v2.2.0+incompatible
)
